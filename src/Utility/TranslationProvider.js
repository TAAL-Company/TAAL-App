...
export const TranslationProvider = ({ children, defaultLang = 'en' }) => {
  ...
  const translate = async (text, targetLang = currentLanguage) => {
    // If showOriginal is enabled, return text as-is without translation
    if (showOriginal) {
      return text;
    }
  ...
  return (
    <TranslationContext.Provider
      value={{ translate, currentLanguage, setLanguage: setCurrentLanguage, showOriginal, setShowOriginal }}
    >
      {children}
    </TranslationContext.Provider>
  );
