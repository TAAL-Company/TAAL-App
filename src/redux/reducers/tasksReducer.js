...
const tasksReducer = (state = initialState, action) => {
  switch (action.type) {
    case type.TASKS_CHANGE:
      ...
      action.payload.user_tasks.forEach((element) => {
        if (objList.hasOwnProperty(element.id)) {
          if (dateHasChanged) objList[element.id]["didFinish"] = null;
    ...
    case type.CURRENT_SITE_TASKS:
      let currentTaskName = action.payload[0][0].title;
      return {
        ...state,
    ...
    case type.CURRENT_SITE_TASKS_LIST:
      const firstTask = action.payload[0] ? action.payload[0].title : "";
      return {
        ...state,
